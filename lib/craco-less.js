...
const overrideWebpackConfig = ({ context, webpackConfig, pluginOptions }) => {
  ...
  const { isFound } = getLoader(webpackConfig, loaderByName("file-loader"));
  if (!isFound) {
    throwError(
  ...
  oneOfRule.oneOf.splice(fileLoaderIndex, 0, lessRule);
  ...
  oneOfRule.oneOf.splice(fileLoaderIndex, 0, lessModuleRule);
