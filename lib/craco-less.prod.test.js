...
} = require("@craco/craco/dist/lib/cra");
...
test("the webpack config is modified correctly without any options", () => {
  ...
  expect(lessModuleRule.use[2].options.postcssOptions.ident).toEqual("postcss");
  expect(
    lessModuleRule.use[2].options.postcssOptions.plugins,
  ).not.toBeUndefined();
...
test("the webpack config is modified correctly with all loader options", () => {
  ...
  expect(lessRule.use[2].loader).toContain(`${path.sep}postcss-loader`);
  expect(lessRule.use[2].options.postcssOptions.ident).toEqual("test-ident");
  expect(lessRule.use[2].options.postcssOptions.plugins).not.toBeUndefined();
  ...
  expect(lessModuleRule.use[2].loader).toContain(`${path.sep}postcss-loader`);
  expect(lessModuleRule.use[2].options.postcssOptions.ident).toEqual(
    "test-ident",
  );
  expect(
    lessModuleRule.use[2].options.postcssOptions.plugins,
...
test("throws an error when we can't find the oneOf rules in the webpack config", () => {
  ...
  expect(runTest).toThrowError(
    "Can't find a 'oneOf' rule under module.rules in the production webpack config!\n\n" +
      ...
      "   * https://github.com/dilanx/craco/issues?q=is%3Aissue+webpack+rules+oneOf\n" +
